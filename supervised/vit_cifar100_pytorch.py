...
class Patches(layers.Layer):
    ...
    def call(self, images):
        ...
        if keras.backend.backend() == "torch":
            x = ops.transpose(images, (0, 3, 1, 2))  # NHWC -> NCHW
            patches = torch.nn.functional.unfold(
                x, kernel_size=self.patch_size, stride=self.patch_size
            )
            return ops.transpose(patches, (0, 2, 1))
...
resized_image = ops.image.resize(
    ops.convert_to_tensor(np.expand_dims(image, axis=0)), size=(image_size, image_size)
)
...
# Save as PDF
plt.savefig("image_patches.pdf", format="pdf", bbox_inches="tight")
...
# Initilaise the wandb
wandb.init(
    project="Keras3-Tutorials",
    name="vit-cifar100",
    entity="sayannath235",
    config=wandb_config,
)
